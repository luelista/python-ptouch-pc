...
class App:
    def closedlg(self, response):
        ...
        self.t.destroy()
    def __init__(self, t, width, height, rgb_colors):
        self.width = width
        self.height = height
        self.t = t
        ...
        ok = Button(t, text="OK", command=lambda: self.closedlg(True)); ok.pack()
        cancel = Button(t, text="Cancel", command=lambda: self.closedlg(False)); cancel.pack()



def displayGraphic(width, height, graphic):
    t = Tk()
    a = App(t, width, height, iter(graphic))
    t.mainloop()
